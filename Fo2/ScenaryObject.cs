...
namespace Fo2
{
    class ScenaryObject
    {
        ...
        public ScenaryObject(Vector2 position, Texture2D texture)
        {
            ...
            if (fileName.Substring(fileName.Length - 4, 4) == "_000") { fileName = fileName.Substring(0, fileName.Length - 4); }
            byte[] bytes = File.ReadAllBytes(fileName + ".inf");

            int x1 = bytes[10];
            int x2 = bytes[11];

            int numberX = (x1 == 255) ? x2-256 : (x1 * 16 * 16 + x2);


            int y1 = bytes[22];
            int y2 = bytes[23];
            int numberY = (y1 == 255) ? y2-256 : (y1 * 16 * 16 + y2);
